...
async def test_create_metrics_returns_metrics_built_from_system_stats():
    ...
    with patch("client.metric_factory.time.time", return_value=123.456), \
         patch("client.metric_factory.util.boot_time", return_value=1000.0), \
         patch("client.metric_factory.util.cpu_count", return_value=16), \
         patch("client.metric_factory.util.cpu_percent", return_value=[10.0, 20.0]), \
    ...
    assert metrics.boot_time == 1000.0
    assert metrics.core_count == 16
    assert metrics.cpu_usage == [10.0, 20.0]
